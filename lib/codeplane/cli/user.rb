    ...
    class User < Base
      def self.help
        Codeplane::CLI.stdout.write <<-TEXT.strip_heredoc
          == User collaboration
             codeplane user:list [REPO]             #{"# list collaborators on given REPO".send(Codeplane.colors)}
             codeplane user:add [REPO] [EMAIL]      #{"# invite EMAIL to join REPO".send(Codeplane.colors)}
             codeplane user:remove [REPO] [EMAIL]   #{"# revoke permission for EMAIL on REPO".send(Codeplane.colors)}
      ...
      def list
        ...
        Codeplane::CLI.stdout << users.collect do |user|
          ...
          "".tap do |s|
            s << user.name
            s << padding << "    "
            s << "# #{user.email}".send(Codeplane.colors)
          end
        end.join("\n") << "\n"
      end
